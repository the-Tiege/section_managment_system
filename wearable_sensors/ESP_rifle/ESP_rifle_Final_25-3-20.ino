...
const int Sleep_time = (5*60); //length of time to sleep in seconds. (5*60) is 5 minutes, when the ESP wakes by sleep timer
...
typedef struct send_struct {
  int sensor = 1;//identifies Which sensor it is. 1 means rifle sensor 2 means body armour sensor
...
unsigned long period = 0;//"period" sets duration of the timer, it is set to 0 here so that the first time the ESP
                          //sends a message there is no timer. this is updated to 5 seconds if the message fails to send.
/////////////////////////////////////////////////////////////////////////////
...
void setup() {
  ...
  if(wakeup_reason == ESP_SLEEP_WAKEUP_TIMER)//If the wake up reason was the Sleep timer, the ESP wakes up the arduino by sending a logic LOW to digital pin 3 of the arduino. This triggers an inturrupt and wakes the arduino.
  {
  ...
  while(Serial2.available()==0)//Waits for arduino to send information via the serial.
   ...
   if(Serial2.read()== 'H')//When it reads the string starting with "H" it stores the information in the structure to be sent.
    ...
    Serial.println("BATT :" + String(test.Battery) + " RNDS : " +String(test.info));//Prints information for debugging.
...
esp_now_register_recv_cb(OnDataRecv);
  ...
  while(!retry)//if auto ack is not recieved send message again
  {
    ...
    if(Serial2.read()== 'H')//If new information is sent from the Sensor while trying to send a message. The information is updated.
    {
      ...
      test.Battery = Serial2.parseInt();
      test.info += Serial2.parseInt();//The "+=" is unique to the Ammunition tracker. If rounds are fired while the ESP is still trying to send its message the new rounds fired will be added to the ones already fired.
      

      Serial2.flush();

      Serial.println("BATT :" + String(test.Battery) + " RNDS : " +String(test.info));
